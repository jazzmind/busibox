"""
Post-Deployment Validation Tests (PVT) for Data Service.
...
# For local testing: Set DATA_API_URL to the remote container's URL
API_PORT = os.getenv("API_PORT", "8002")
SERVICE_URL = os.getenv("DATA_API_URL", f"http://localhost:{API_PORT}")
...
def access_token(auth_client):
    """Get an access token for the data API using user-scoped token exchange."""
    return auth_client.get_token(audience="data-api")


@pytest.fixture(scope="module")
def auth_headers(auth_client):
    """Return headers with Bearer token and X-Test-Mode."""
    return auth_client.get_auth_header(audience="data-api")
