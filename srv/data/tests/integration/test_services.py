    ...
    try:
        ...
        async with rls_pool.acquire() as conn:
            ...
            row = await conn.fetchrow(
                "SELECT * FROM data_files WHERE file_id = $1",
    ...
    try:
        ...
        async with rls_pool.acquire() as conn:
            file_row = await conn.fetchrow(
                "SELECT file_id, filename, content_hash FROM data_files WHERE file_id = $1",
                uuid.UUID(test_file_id)
            )
        ...
        await minio_service.delete_file(storage_path)
        async with rls_pool.acquire() as conn:
            await conn.execute("DELETE FROM data_files WHERE file_id = $1", uuid.UUID(test_file_id))
