...
Tests Marker-based PDF extraction by triggering the actual deployed data worker
and verifying:
...
Or directly on the data server:
    cd /srv/data && source venv/bin/activate
...
def get_worker_logs(since_time: Optional[datetime] = None, lines: int = 500) -> str:
    """Get data-worker logs from journalctl."""
    try:
        cmd = ["journalctl", "-u", "data-worker", "-n", str(lines), "--no-pager", "-o", "cat"]
        if since_time:
...
def test_pdf_extraction_marker():
    ...
    try:
        result = subprocess.run(
            ["systemctl", "is-active", "data-worker"],
            capture_output=True, text=True, timeout=5
        )
        worker_status = result.stdout.strip()
        print(f"Worker status: {worker_status}")
        if worker_status != "active":
            print("⚠ Warning: data-worker is not active, test may use local extraction only")
    except Exception as e:
