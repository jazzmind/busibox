...
Environment variables:
    DATA_DB_URL: Data database connection URL
...
Execution Context:
    - Runs on: Admin workstation or within data container
...
def get_data_db_url() -> str:
    """Get data database connection URL."""
    if url := os.environ.get("DATA_DB_URL"):
        return url
    
    # Try DATA_DB_* first, then fall back to POSTGRES_* (used in containers)
    host = os.environ.get("DATA_DB_HOST") or os.environ.get("POSTGRES_HOST", "10.96.200.206")
    port = os.environ.get("DATA_DB_PORT") or os.environ.get("POSTGRES_PORT", "5432")
    dbname = os.environ.get("DATA_DB_NAME") or os.environ.get("POSTGRES_DB", "data")
    user = os.environ.get("DATA_DB_USER") or os.environ.get("POSTGRES_USER", "data")
    password = os.environ.get("DATA_DB_PASSWORD") or os.environ.get("POSTGRES_PASSWORD", "data")
...
async def mark_file_encrypted(
    ...
    await conn.execute(
        "UPDATE data_files SET is_encrypted = true WHERE file_id = $1",
        file_id
    )
