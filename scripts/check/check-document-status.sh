...
if [ -n "$1" ]; then
  ...
  ssh root@${DATA_IP} "psql -U busibox_user -d files -c \"SELECT file_id, filename, stage, progress, chunks_processed, total_chunks, error_message, updated_at FROM data_status WHERE file_id = '$FILE_ID';\""
else
  echo "=== Recent Files (Last 10) ==="
  ssh root@${DATA_IP} "psql -U busibox_user -d files -c \"SELECT file_id, filename, stage, progress, chunks_processed, total_chunks, error_message, updated_at FROM data_status ORDER BY updated_at DESC LIMIT 10;\" | head -30"
fi

echo ""
echo "=== Worker Status ==="
ssh root@${DATA_IP} "systemctl status data-worker --no-pager | head -10"

echo ""
echo "=== Recent Worker Logs (Last 10 lines) ==="
ssh root@${DATA_IP} "journalctl -u data-worker -n 10 --no-pager | grep -E 'event|error|stage|embedding|chunk' || journalctl -u data-worker -n 10 --no-pager"
...
ssh root@${DATA_IP} "redis-cli XLEN jobs:data"

echo ""
echo "================================"
echo "To watch live logs:"
echo "  Worker:   ssh root@${DATA_IP} 'journalctl -u data-worker -f'"
